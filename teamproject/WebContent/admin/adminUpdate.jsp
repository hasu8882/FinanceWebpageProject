		...
		<div class="col-8">
			<form action="adminUpdate.do" method="post" name="frm">
				...
				<div class="form-floating mb-3">
					...
					<c:if test="${member.pay == 0}">
					<div class="form-floating mb-3">
					<h5 class="fw-bold mb-0">회원 유형</h5>
					<input type="radio" name="pay" value=1 >유료 
					<input type="radio" name="pay" value=0 checked>무료 <br />
				</div>
					</c:if>
					 <c:if test="${member.pay == 1}">
					<div class="form-floating mb-3">
					<h5 class="fw-bold mb-0">회원 유형</h5>
					<input type="radio" name="pay" value=1 checked>유료 
					<input type="radio" name="pay" value=0>무료 <br />
				</div>
					</c:if>
				 <c:if test="${member.admin == 1}">
				<div class="form-floating mb-3">
					<h5 class="fw-bold mb-0">관리자 권한</h5>
					<input type="radio" name="admin" value=0 >관리자 <input
						type="radio" name="admin" value=1 checked>회원 <br />
				</div>
				</c:if>
				<c:if test="${member.admin == 0}">
				<div class="form-floating mb-3">
					<h5 class="fw-bold mb-0">관리자 권한</h5>
					<input type="radio" name="admin" value=0 checked>관리자 <input
						type="radio" name="admin" value=1 >회원 <br />
				</div>
				</c:if>
				<input type="hidden"  name="pw" value="${member.pw}">
				<div class="row g-3">
					<input type="button" class="w-50 mb-2 btn btn-sm rounded-4 btn-primary"
						  onclick="location.href='deleteAdmin.do?id=${member.id}'" value="회원 삭제">
